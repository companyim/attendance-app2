...
export default function GradeAttendanceCheck() {
  ...
  useEffect(() => {
    async function fetchAttendance() {
      ...
      try {
        ...
        params.append('grade', selectedGrade);
        params.append('type', 'grade'); // 학년 출석 타입
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(`학년 출석이 저장되었습니다. (${successCount}명, 출석자 달란트 +1)`);
    } catch (error: any) {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-xl font-bold">학년 출석 체크</h2>
        <span className="text-sm text-green-600 bg-green-100 px-2 py-1 rounded">출석 시 달란트 +1</span>
      </div>
